...
class Server:
    ...
    def write_logSite(self,msg):
        with open("log/hostlog.txt","a+") as file:
            file.write(msg)
            file.write("\n")

    def isinblacklist(self,site,blacklist):
        found = False
        for bsite in blacklist:
            if bsite in site:
                found = True
                break
        return found
    ...
    def start_server(self, conn=5, buffer=4096, port=8080):
        try:
            ...
            self.write_logSite(self.getTimeStampp() + "   \n\nStarting Server\n\n")
    ...
    def connection_read_request(self, conn, addr, buffer):
        ...
        try:
            ...
            try:
                self.write_logSite(self.getTimeStampp()+": "+ webserver.decode("utf-8") )
                #if target in self.blacklist_websites_lookup:
                if self.isinblacklist(webserver.decode("utf-8"),self.blacklist_websites_lookup):
                    print(self.getTimeStampp() + "   Website Blacklisted")
                    self.write_log(self.getTimeStampp() + "   Website Blacklisted")
                    self.write_logSite(self.getTimeStampp()+" [Blacklisted] : "+ webserver.decode("utf-8") )
                    conn.close()
            except Exception as error:
                 self.write_log(self.getTimeStampp() + " [Error] " + error)
                 print(self.getTimeStampp() + " [Error]" + error)
...
if __name__ == "__main__":
    ...
    server = Server(['127.0.0.81'],['facebook','googleadservices','amazon-adsystem','doubleclick','amazon', 'flipkart`'])
    server.start_server()
